...
- new `lake-check-sandbox` input (`"none"` | `"sysctl"` | `"setuid"`, default `"none"`) choosing how to make bubblewrap's sandbox able to start. Ubuntu 24.04 and newer block the unprivileged user namespaces it needs, so GitHub-hosted runners need `"sysctl"` or `"setuid"`; `lean-action` never changes the runner's configuration unless asked, and only when the sandbox is found not to work. Namespace runners instead need a privileged container (`namespace-features:container.privileged=true` with a `-with-features` machine label), after which no `lake-check-sandbox` value is needed
